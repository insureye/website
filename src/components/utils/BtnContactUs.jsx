...
const BtnContactUs = ({name, link, reload=true}) => {
    return(
    <div className={`${styles.btnContactUs} ${styles.btnContactUsHover} flex flex-col items-center justify-center rounded-full p-[1px] min-w-[100px] max-w-[146px]`}>
        <Link to={link} onClick={() => reload ? window.location = "#ContactUs" : null} className={`py-2 px-4 w-full h-full rounded-full text-[17px] text-center`}>
            {name}
        </Link>
    </div>);
}
